...
import com.chinacache.stormkafka.KafkaSpout.MessageAndRealOffset;
import com.chinacache.stormkafka.trident.MaxMetric;
...
public class PartitionManager {
    ...
    static {
        if (ipMap.keySet().size() == 0) {

			ipMap.put("180.97.185.50", "10.20.72.139");
			ipMap.put("180.97.185.51", "10.20.72.140");
			ipMap.put("180.97.185.52", "10.20.72.141");
			ipMap.put("180.97.185.53", "10.20.72.142");
			ipMap.put("180.97.185.54", "10.20.72.143");
			ipMap.put("180.97.185.55", "10.20.72.144");
			ipMap.put("180.97.185.66", "10.20.72.149");
			ipMap.put("180.97.185.67", "10.20.72.150");
			ipMap.put("180.97.185.68", "10.20.72.151");
			ipMap.put("180.97.185.69", "10.20.72.152");

			ipMap.put("153.99.250.42", "10.20.72.145");
			ipMap.put("153.99.250.43", "10.20.72.146");
			ipMap.put("153.99.250.44", "10.20.72.147");
			ipMap.put("153.99.250.45", "10.20.72.148");
			ipMap.put("153.99.250.50", "10.20.72.155");
			ipMap.put("153.99.250.51", "10.20.72.156");
			ipMap.put("153.99.250.52", "10.20.72.157");
			ipMap.put("153.99.250.53", "10.20.72.158");
        }
    }
    ...
    public PartitionManager(DynamicPartitionConnections connections, String topologyInstanceId, ZkState state,
        ...
        _partition = new Partition(new Broker((String) ipMap.get(id.host.host), id.host.port), id.partition);
        _connections = connections;
        _spoutConfig = spoutConfig;
        _topologyInstanceId = topologyInstanceId;
        _consumer = connections.register(new Broker((String) ipMap.get(id.host.host), id.host.port), id.partition);
        _state = state;
        ...
        if (currentOffset - _committedTo > spoutConfig.maxOffsetBehind || _committedTo <= 0) {
            ...
            _committedTo = currentOffset;
            LOG.info("Commit offset " + _committedTo + " is more than " + spoutConfig.maxOffsetBehind
                    + " behind, resetting to startOffsetTime=" + spoutConfig.startOffsetTime);
        }
    ...
    public void fail(Long offset) {
        if (offset < _emittedToOffset - _spoutConfig.maxOffsetBehind) {
            LOG.info("Skipping failed tuple at offset=" + offset + " because it's more than maxOffsetBehind="
                    + _spoutConfig.maxOffsetBehind + " behind _emittedToOffset=" + _emittedToOffset);
        } else {
            LOG.debug("failing at offset=" + offset + " with _pending.size()=" + _pending.size()
                    + " pending and _emittedToOffset=" + _emittedToOffset);
            failed.add(offset);
            numberFailed++;
    ...
    public void commit() {
        ...
        if (_committedTo != lastCompletedOffset || _committedTo == lastCompletedOffset) {
            LOG.debug("Writing last completed offset (" + lastCompletedOffset + ") to ZK for " + _partition
            ...
            Map<Object, Object> data = (Map<Object, Object>) ImmutableMap
                    ...
                    .put("offset", lastCompletedOffset).put("partition", _partition.partition)
                    .put("broker", ImmutableMap.of("host", _partition.host.host, "port", _partition.host.port))
                    .put("topic", _spoutConfig.topic).build();
            // System.out.println("11111111111111111111111111" + committedPath() + " " + data);
            _state.writeJSON(committedPath(), data);

            _committedTo = lastCompletedOffset;
            LOG.debug("Wrote last completed offset (" + lastCompletedOffset + ") to ZK for " + _partition
                    + " for topology: " + _topologyInstanceId);
        } else {
